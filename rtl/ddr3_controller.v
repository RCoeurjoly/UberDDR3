...
module ddr3_controller #(
    ...
    (
        ...
        output	wire	[13:0]	o_debug_calib_search,
...
//        output	wire	[31:0]	o_debug3
   ...
   localparam tRCD = (SPEED_BIN == 0) ? TRCD :  //  use top-level parameters
    ...
    reg[1:0] check_starting_data_invalid_phase = 0;
    reg check_starting_data_seen_valid = 0;
    reg check_starting_data_invalid_seen = 0;
    reg[5:0] check_starting_data_first_invalid_index = 0;
    ...
    reg[5:0] debug_calib_abort_best_offset = 0;
    reg[5:0] debug_calib_abort_best_distance = 0;
    reg debug_calib_abort_best_accepted = 0;
    ...
    always @(posedge i_controller_clk) begin
        if(!i_rst_n) begin
            ...
            debug_calib_abort_best_offset <= 6'd0;
            debug_calib_abort_best_distance <= 6'd0;
            debug_calib_abort_best_accepted <= 1'b0;
            ...
            if(calib_abort_snapshot_valid && !debug_calib_abort_seen) begin
                ...
                debug_calib_abort_best_offset <= calib_abort_snapshot_best_offset;
                debug_calib_abort_best_distance <= calib_abort_snapshot_best_distance;
                debug_calib_abort_best_accepted <= calib_abort_snapshot_best_accepted;
            end
...
//    assign o_phy_dq_tri_control = !write_dq[STAGE2_DATA_DEPTH]; // Warning in implementation: not routable to load
    ...
    always @(posedge i_controller_clk) begin
        if(sync_rst_controller) begin
            ...
            calib_abort_snapshot_read_lane_data_shifted <= 32'd0;
...
ANALYZE_DATA_LOW_FREQ: if(DLL_OFF) begin // read_data_store should have the expected 9177298cd0ad51c1, if not then issue bitslip
        ...
        ANALYZE_DATA:   if(prep_done[1]) begin
                            if(write_pattern_matches) begin
                                ...
                                if(read_lane_data_shifted_invalid) begin
                                    ...
                                    else if(analyze_data_recenter_retry[lane] != 2'd3) begin
                                        analyze_data_search_offset_q <= 6'd0;
 ...
 ANALYZE_DATA_INVALID_WINDOW_RECENTER: begin
                            ...
                            else if(analyze_data_contradiction_retry != 2'd3) begin
                                analyze_data_contradiction_retry <= analyze_data_contradiction_retry + 1'b1;
                                analyze_data_search_invalid <= 1'b0;
                                analyze_data_invalid_retry[lane] <= 0;
                                analyze_data_recenter_retry[lane] <= 0;
 ...
 ANALYZE_DATA_SEARCH: begin
                            if(hamming32(analyze_data_search_window[analyze_data_search_offset_q +: 32] ^ write_pattern[0 +: 32]) < analyze_data_search_best_distance) begin
                                analyze_data_search_best_offset <= analyze_data_search_offset_q;
                                analyze_data_search_best_distance <= hamming32(analyze_data_search_window[analyze_data_search_offset_q +: 32] ^ write_pattern[0 +: 32]);
                            ...
                            if(analyze_data_search_offset_q == 6'd32) begin
                                ...
                                analyze_data_search_offset_q <= analyze_data_search_offset_q + 6'd8;
                            end
                        end
 ...
 ANALYZE_DATA_SEARCH_DONE: begin
                            ...
                            debug_calib_search_best_accepted <= (analyze_data_search_best_distance <= 6'd6) &&
                                                                 (analyze_data_search_best_offset + 6'd8 >= start_index_check) &&
                                                                 (analyze_data_search_best_offset <= start_index_check + 6'd8);
...
//    wire debug_trigger;
    ...
    assign o_debug_calib_abort = {
        debug_calib_abort_read_lane_data_shifted,
        2'd0,
        debug_calib_abort_shifted_match,
        debug_calib_abort_lane_read_dq_early,
        debug_calib_abort_lane_write_dq_late,
        debug_calib_abort_start_index_check[5:0],
        debug_calib_abort_dq_target_index[5:0],
        debug_calib_abort_data_start_index[5:0],
        debug_calib_abort_lane[3:0],
                ...
                `ifdef FORMAL_CALIBRATION_FAILURES
                    if(!$past(past_sync_rst_controller)) begin
                        ...
                        if($past(state_calibrate) == ANALYZE_DATA && $past(prep_done[1]) &&
                           ...
                           $past(read_lane_data_shifted_invalid)) begin
                            assert(!reset_from_calibrate);
                            assert(state_calibrate == ANALYZE_DATA_INVALID_WINDOW_RECENTER);
                        end

                        if($past(state_calibrate) == ANALYZE_DATA_INVALID_WINDOW_RECENTER &&
