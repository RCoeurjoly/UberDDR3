...
A first stripped version of the same wrapper, built with
...
That first stripped build also exposed a Makefile dependency trap: when
`CALIB_PRE_PLACE_BEL_LOCKS` is set to an artifact path, the Makefile can treat
that path as the generated `v40-calib-phy-pre-place-bel-locks.py` target and
overwrite it from the 411-lock physical baseline. The command-enabled fullbeat
experiment really applied the intended hard + `RDY_AND_LUT_1` lock set
(`applied=525 missing=28`), but the first stripped build applied only the old
411 physical locks. Regenerate the desired script before these experiments and
set `ROWSTREAM_FULLBEAT_PRE_PLACE_DEPS=` when the pre-place script is an
external artifact that must not be regenerated by make.

After regenerating the hard + `RDY_AND_LUT_1` allow-missing script and building
the stripped wrapper with `ROWSTREAM_FULLBEAT_PRE_PLACE_DEPS=`, nextpnr applied
the intended lock set (`applied=525 missing=28`) and produced:

```text
artifacts/manual-seed/fullbeat-v91-v88hard-lut1-cmdalloff-fixeddeps/seed5/rowstream-fullbeat-v91-v88hard-lut1-cmdalloff-fixeddeps-seed5.bit
```

The corrected artifact still did not calibrate. Hardware again reported
`version=89`, `state=ANALYZE_DQS`, instruction `13`, `debug1=0x000015a4`,
`ack_count=0`, and `calib_seen_cycle=0x00000000`. This closes the easy
stripped-wrapper explanation: applying the intended 553-lock oracle is not
